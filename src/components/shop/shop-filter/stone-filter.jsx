...
const StoneFilter = ({ setCurrPage, shop_right = false, finishFilterData }) => {
  const { data: finishData, isError, isLoading } = useGetStoneListQuery();
  ...
  useEffect(() => {
    if (
      finishData &&
      finishData?.data &&
      finishData?.data?.productStoneTypes &&
      finishData?.data?.productStoneTypes?.edges
    ) {
      const list = finishData?.data?.productStoneTypes?.edges;
      if (list?.length > 0) {
